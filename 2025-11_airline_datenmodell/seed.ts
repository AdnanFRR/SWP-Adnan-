...
let passengerAttempt = 0; // globale Versuchszahl, damit wir bei jedem Versuch eine eindeutige E-Mail haben
while (passengers_created < passengers_to_create) {
    passengerAttempt++;
    // generiere Namen einmal pro Versuch
    const firstName = faker.person.firstName();
    const lastName = faker.person.lastName();
    // Erzeuge eine garantiert eindeutige E-Mail für diesen Versuch, indem wir einen Zähler
    // und Zeitstempel anhängen. Das verhindert Unique-Constraint-Fehler beim Seeder.
    const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}.${Date.now()}.${passengerAttempt}@example.com`;
    try {
        ...
        // Sollte trotzdem ein Fehler auftreten (z. B. DB-Problem), loggen wir ihn und
        // fahren mit dem nächsten Versuch fort. Dank der eindeutigen E-Mail sollte
        // ein doppelter Eintrag wegen E-Mail selten bis nie vorkommen.
